...
class BaseClass(pygame.sprite.Sprite):
    ...
    allsprites = pygame.sprite.Group()# similer to python list
    ...
    def update_all(SCREENWIDTH, SCREENHEIGHT):
        for fly in Enemy.List:
            ...
            if fly.health <= 0: # is dead
                fly.velx = 0 # this fly is now paralyzed

                if fly.rect.y + fly.rect.height < SCREENHEIGHT: # checking if fly is above the ground

                    fly.rect.y += fly.vely # moves down
            else:
                fly.fly(SCREENWIDTH)
